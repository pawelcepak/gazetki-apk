...
LEAFLETS_OUT=Path('data/leaflets.json'); PRODUCTS_OUT=Path('data/products.json'); PAGE_INDEX_OUT=Path('data/page-index.json')
...
def products_from_leaflet(raw,leaflet):
 ...
 for i,c in enumerate(chunks):
  ...
  promo_match=re.search(r'Cena promocyjna:\s*(\d{1,4}[,.]\d{2})\s*zł',detail,re.I);promo=float(promo_match.group(1).replace(',','.')) if promo_match else None
...
def adapter(store):
 ...
 a,b=dr;leaf={'id':f'{store}-{a}-{b}','storeId':store,'store':NAMES[store],'title':TITLES[store],'from':a,'to':b,'source':'mojagazetka','sourcePage':source,'externalUrl':OFFICIAL[store],'reader':'images','pageCount':len(pages),'pages':pages}
 return leaf,products_from_leaflet(raw,leaf)
def load_old_index():
...
def ocr_page(url):
 ...
 try:
  p=subprocess.run(['tesseract',path,'stdout','-l','pol','--psm','11'],capture_output=True,text=True,timeout=45)
  if p.returncode:return ''
  return re.sub(r'\s+',' ',p.stdout).strip()[:12000]
 finally:
  try:os.unlink(path)
  except:pass
def build_page_index(leaflets):
 ...
 for leaf in leaflets:
  for n,url in enumerate(leaf['pages'],1):
   ...
   if prev and prev.get('imageUrl')==url and prev.get('text'):
    out.append(prev);continue
   txt=''
   if can_ocr:
    try:txt=ocr_page(url)
    except Exception as e:print(f'OCR warning {leaf["storeId"]} p{n}: {e}')
   out.append({'leafletId':leaf['id'],'storeId':leaf['storeId'],'store':leaf['store'],'page':n,'imageUrl':url,'text':txt})
 return out
def main():
 ...
 PRODUCTS_OUT.write_text(json.dumps({'schemaVersion':4,'generatedAt':now,'demo':False,'products':products},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 PAGE_INDEX_OUT.write_text(json.dumps({'schemaVersion':1,'generatedAt':now,'pages':page_index},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print(f'Wrote {len(items)} leaflets; pages={sum(len(x.get("pages",[])) for x in items)}; products={len(products)}; indexedPages={sum(bool(x.get("text")) for x in page_index)}; errors={len(errors)}')
